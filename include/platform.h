...
struct Platform
{
    ...
    void PlayBeep(uint8_t SoundTimer){
        if(audioEnabled && SoundTimer > 0)
        {
            ...
            int16_t* samples = new int16_t[44100 / 60];
            for(int i = 0; i < 44100 / 60; i++)
            {
                samples[i] = AMPLITUDE * sin(2 * 3.14159265358979323846 * FREQUENCY * i / 44100.0);
            }
            
            SDL_QueueAudio(audioDevice, samples, (44100 / 60) * sizeof(int16_t));
            delete[] samples;
        }
